...
console.log('🚀 Сайт загружен. Версия скрипта: 2.3.0');
...
async function loadCatalogData() {
  try {
    ...
    // Возвращаем данные по умолчанию если файл недоступен
    return {
      categories: [
      ...
      products: [
        {
          ...
          price: 14500,
          oldPrice: 21000,
          description: 'Современная тумба для ТВ с качественной фурнитурой и надежной конструкцией',
          imageUrl: 'https://images.unsplash.com/photo-1556901587-7c176b8a7f8a?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
          badge: '-30%',
          inStock: true
        },
        {
          ...
          price: 6400,
          description: 'Компактная тумба для хранения документов и аксессуаров',
          imageUrl: 'https://images.unsplash.com/photo-1556906149-462b99e62c0f?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
          inStock: true
        },
        {
          id: 'sink-1',
          category: 'equipment',
          name: 'Мойка с сифоном',
          price: 1200,
          description: 'Надежная мойка для кухонь и санузлов с комплектом сифона',
          imageUrl: 'https://images.unsplash.com/photo-1584622650111-993a426fbf0a?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80',
          inStock: true
        }
      ]
    };
...
async function renderCatalog() {
  console.log('🔄 Отображение каталога');
  
  const catalogData = await loadCatalogData();
  const container = document.querySelector('.products-grid');
  
  if (!container) {
    console.warn('⚠️ Контейнер для каталога не найден');
    return;
  }
  ...
  catalogData.products.forEach(product => {
    if (!product.inStock) return; // Пропускаем товары не в наличии
  ...
  initAddToCartButtons(); // Инициализируем кнопки после добавления товаров
}
  ...
  catalogData.categories.forEach(category => {
    ...
    categoryCard.innerHTML = `
      <div class="category-image">
        <img src="https://via.placeholder.com/300x200/3498db/ffffff?text=${encodeURIComponent(category.name)}" alt="${category.name}" loading="lazy">
      </div>
...
function initNavigation() {
  ...
  document.querySelectorAll('.nav-btn').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      // Если это не ссылка на внешний ресурс, показываем контент
      const href = this.getAttribute('href');
      if (href && !href.startsWith('http') && !href.startsWith('tel:') && !href.startsWith('mailto:')) {
        // Здесь можно добавить логику для SPA навигации
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Если это страница каталога, загружаем данные
  ...
  setTimeout(() => {
    ...
    // Показываем кнопку только для администратора (для простоты - всегда)
